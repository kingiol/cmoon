...
int okol_data_get(CGI *cgi, HASH *dbh, HASH *evth, session_t *ses)
{
	...
	mcs_build_querycond_i(hdf_get_obj(cgi->hdf, PRE_QUERY),
						  ...
						  &str);
	if (str.len <= 0) string_append(&str, "intime > current_date - 2");
	else string_append(&str, " 1 ");

	/*
	 * execute
	 */
	MDB_QUERY_RAW(conn, "stat", STAT_COL, "%s", NULL, str.buf);
	string_clear(&str);
	mdb_set_rows(cgi->hdf, conn, STAT_COL, PRE_OUTPUT, 0);
